...
s=re.sub(r'^changelog=.*$',f'changelog={pack_rev}：跟随 MeTube 上游 {version}；GitHub Actions 构建阶段将经过 SHA256 校验的 FFmpeg 7.0.2-static 直接打入 FPK，fnOS 安装阶段不下载 FFmpeg。',s,flags=re.M)
